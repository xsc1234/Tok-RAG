...
class GreedySearchDecoderOnlyOutput(ModelOutput):
    ...
    dic_token_to_js: Optional[Dict[int, int]]  = None
    list_token_and_logits: Optional[Dict[int, int]]  = None
    decision_list: Optional[Dict[int, int]] = None
    model_kwargs: Optional[Dict[int, int]] = None

class GreedySearchDecoderOnlyOutputAttentions(ModelOutput):
    """
    Base class for outputs of decoder-only generation models using greedy search.
    ...
    hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    premature_layer_dist: Optional[Dict[int, int]] = None
    dic_token_to_js: Optional[Dict[int, Dict[str,Tuple[torch.FloatTensor]]]] = None
...
class GenerationMixin:
    ...
    def generate(
        ...
        attention_add: Optional[torch.Tensor] = None,
        ref_start_idx: int = None,
        ref_end_idx: int = None,
        constrain_decode: Optional[torch.Tensor] = None,
        passage_emb: Optional[torch.Tensor] = None,
        rag_len = None,
        past_rag_kv: Optional[List[torch.FloatTensor]] = None,
        soft_num = None,
        scale: Optional[int] = 0.5,
        layer_start: Optional[int] = 0,
        icl_s_idx=None,
        query_s_idx=None,
        selected_layer=None,
        query_rag_end_idx=None,
        query_icl_end_idx=None,
        query_end_idx=None,
        first_token=None,
        get_rag_emb=None,
        dic_rag_att=None,
        **kwargs,
        ...
        if is_greedy_gen_mode and dola_decoding:
            ...
            if no_rag_ids is not None:
                return self.dola_greedy_decode_multi_ids(
                    input_ids=input_ids,
                    no_rag_ids=no_rag_ids,
                    rag_ids_list=rag_ids_list,
                    word_embedding_layer=word_embedding_layer,
                    model_kwargs_no_rag=model_kwargs_no_rag,
                    rate=rate,
                    tokenizer=tokenizer,
                    logits_processor=logits_processor,
                    stopping_criteria=stopping_criteria,
                ...
                return self.dola_greedy_decode_output_attentions(
                    input_ids,
                    tokenizer=tokenizer,
                    logits_processor=logits_processor,
                    stopping_criteria=stopping_criteria,
            ...
            #     **model_kwargs,
            # )
        elif is_greedy_gen_mode and selected_layer is not None:
            return self.greedy_search_batch_icl(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                ...
                query_rag_end_idx=query_rag_end_idx,
                query_icl_end_idx=query_icl_end_idx,
                query_end_idx=query_end_idx,
                **model_kwargs,
            )
        elif is_greedy_gen_mode and rag_hidden:
            return self.greedy_search_rag_hidden(
                input_ids,
                rag_hidden,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
        ...
        elif is_greedy_gen_mode and constrain_decode is not None and first_token is not None:
            return self.greedy_search_constrain_decode_first_token(
                input_ids,
        ...
        elif is_greedy_gen_mode and constrain_decode is not None and soft_num is not None:
            return self.greedy_search_constrain_decode_soft(
                input_ids,
                constrain_decode=constrain_decode,
                soft_num=soft_num,
                logits_processor=logits_processor,
        ...
        elif is_greedy_gen_mode and constrain_decode is not None and passage_emb is not None:
            return self.greedy_search_dynamic_constrain_decode(
                input_ids,
                constrain_decode=constrain_decode,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                ...
                rag_len=rag_len,
                past_rag_kv=past_rag_kv,
                return_dict_in_generate=generation_config.return_dict_in_generate,
                synced_gpus=synced_gpus,
                streamer=streamer,
                **model_kwargs,
            )
        elif is_greedy_gen_mode and constrain_decode is not None and dic_rag_att is not None:
            return self.greedy_search_constrain_decode_att(
                input_ids,
                constrain_decode=constrain_decode,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                ...
                synced_gpus=synced_gpus,
                streamer=streamer,
                dic_rag_att=dic_rag_att,
                **model_kwargs,
            )
        elif is_greedy_gen_mode and constrain_decode is not None:
            return self.greedy_search_constrain_decode(
                input_ids,
                constrain_decode=constrain_decode,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
        ...
        elif is_greedy_gen_mode and additional_ids is not None:
            return self.greedy_search_merge(
                input_ids,
        ...
        elif is_greedy_gen_mode and icl_s_idx is not None:
            return self.greedy_search_dual_lora(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
                ...
                streamer=streamer,
                icl_s_idx=icl_s_idx,
                query_s_idx=query_s_idx,
                **model_kwargs,
            )
        elif is_greedy_gen_mode and get_rag_emb is not None:
            return self.greedy_search_one_token(
                input_ids,
                logits_processor=logits_processor,
                stopping_criteria=stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def dola_greedy_decode(
        ...
        elif candidate_premature_layers is not None and mature_layer is None:
        ...
        seq_i = 0
        dic_token_to_js = {}
        list_token_and_logits = []
        init_model_kwargs = model_kwargs
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
            ...
            if base_layer is not None:
                ...
                dic_dola = {}
                dic_first_token = {}
                for idx in range(len(candidate_premature_layers)-1):
                    pre_layers = [candidate_premature_layers[idx]]
                    post_layer = candidate_premature_layers[idx+1]
                    stacked_premature_layers = torch.stack([dict_outputs[i][:, -1, :] for i in pre_layers],
                                                           dim=0)

                    # 2. Calculate the softmax values for mature_layer and all premature_layers
                    softmax_mature_layer = F.softmax(dict_outputs[post_layer][:, -1, :],
                    ...
                    #layer_token = torch.argmax(softmax_premature_layers[0], dim=-1)
                    values_att, indices_att = torch.topk(softmax_premature_layers[0][0], k=3, dim=-1)
                    dic_first_token['{}'.format(pre_layers[0])] = (tokenizer.convert_ids_to_tokens(indices_att), values_att.cpu().numpy().tolist())
                    ...
                    log_softmax_mature_layer = F.log_softmax(dict_outputs[post_layer][:, -1, :],
                                                             dim=-1)  # shape: (batch_size, num_features)
                    ...
                    # print('soft max mature layer')
                    # print(softmax_mature_layer)
                    ...
                    # print(seq_i)
                    # print(output)
                    dic_token_to_js[seq_i] = {output:(js_divs.cpu(),softmax_mature_layer.cpu())}
                    seq_i += 1
                    dic_dola['{}-{}'.format(pre_layers[0],post_layer)] = js_divs[0].item()
                soft_logits = F.softmax(outputs.logits[:, -1, :],dim=-1)
                temp = {}
                temp['logits'] = soft_logits
                temp['token'] = [hs[0][-1] for hs in outputs.hidden_states]
                temp['att'] = [at for at in outputs.attentions] #[1, 32, 1, num_tokens]
                # dola_dic = {}
                # for num,layer_idx in enumerate(candidate_premature_layers):
                #     dola_dic[layer_idx] = js_divs[num]
                temp['dola'] = dic_dola
                temp['token'] = dic_first_token
                list_token_and_logits.append(temp)
            elif candidate_premature_layers is not None:
                # 1. Stacking all premature_layers into a new dimension
                ...
                temp['att'] = [at for at in outputs.attentions] #[1, 32, 1, num_tokens]
                list_token_and_logits.append(temp)
            #print(list_token_and_logits[-1]['token'].shape) torch.Size([4096])
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                return GreedySearchDecoderOnlyOutput(
                    ...
                    dic_token_to_js=dic_token_to_js,
                    list_token_and_logits=list_token_and_logits,
    ...
    def get_predict_emb_contrastive(self,rag_token, indices_att, rag_ids_tensor, tokenizer, topk, values_rag, indices_rag,
                                    word_embedding_layer, no_rag_temp,rate):
        ...
        if no_rag_temp is None:
            ...
            # print(tokenizer.convert_ids_to_tokens(indices_no_rag))
            # print(values_no_rag)
            no_rag_top1_embeddings = []
            for idx_no_rag_token in range(len(indices_no_rag)):
                no_rag_top1_embeddings.append(
                    values_no_rag[idx_no_rag_token] * word_embedding_layer(
                        indices_no_rag[idx_no_rag_token].unsqueeze(0)))
        ...
        for idx in indices_att:
            # print(tokenizer.convert_ids_to_tokens([rag_ids_tensor[idx]]))
            ...
            for att in att_embeddins:
                ...
                #print('cos_sim is {}'.format(cos_sim))
            count += 1
        #print('no rag {} vs rag {} '.format(no_rag_score, max_rag_score))
        if rate * max_rag_score > no_rag_score:
            return True
        else:
            return False

    def dola_greedy_decode_multi_ids(
            self,
            input_ids: torch.LongTensor,
            no_rag_ids: torch.LongTensor,
            rag_ids_list,
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_inputs_no_rag = self.prepare_inputs_for_generation(no_rag_ids, **model_kwargs_no_rag)
            ...
            if base_layer is not None:
                ...
                output = tokenizer.decode(next_tokens)
                if output in filter_words:
                    class_wudao = 1
                else:
                    soft_logits_no_rag = F.softmax(outputs_no_rag.logits[:, -1, :],dim=-1)
                    temp_no_rag = {}
                    temp_no_rag['logits'] = soft_logits_no_rag
                    temp_no_rag['token'] = [hs[0][-1] for hs in outputs_no_rag.hidden_states]
                    temp_no_rag['att'] = [at for at in outputs_no_rag.attentions] #[1, 32, 1, num_tokens]

                    rag_att = temp['att']  # [1,32,1,num_tokens]
                    ...
                    values_att, indices_att = torch.topk(middle_att, 100)
                    rag_ids_tensor = torch.tensor(rag_ids_list, dtype=torch.long).to(input_ids.device)
                    tokens = rag_ids_tensor[indices_att]
                    #print('att top 20 tokens:')
                    values_rag, indices_rag = torch.topk(temp['logits'][0], 2)
                    rag_tokens = [tok.lower() for tok in tokenizer.convert_ids_to_tokens(indices_rag)]
                    if self.get_predict_emb_contrastive(rag_tokens[0], indices_att, rag_ids_tensor, tokenizer, topk,
                                                   values_rag=values_rag,
                                                   indices_rag=indices_rag, word_embedding_layer=word_embedding_layer,
                                                   no_rag_temp=temp_no_rag,rate=rate):
                        class_wudao = 1
                    else:
                        class_wudao = 0
            #print(list_token_and_logits[-1]['token'].shape) torch.Size([4096])
            if class_wudao: #没误导
                ...
                next_token_logits = outputs_no_rag.logits[:, -1, :]
            # pre-process distribution
            next_tokens_scores = logits_processor(input_ids, next_token_logits)
            decision_list.append(class_wudao)
            # Store scores, attentions and hidden_states when required
            if return_dict_in_generate:
                if output_scores:
                    scores += (next_tokens_scores,)
            ...
            # update generated ids, model inputs, and length for next step
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            no_rag_ids = torch.cat([no_rag_ids, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            ...
            model_kwargs_no_rag = self._update_model_kwargs_for_generation(
                outputs_no_rag, model_kwargs_no_rag, is_encoder_decoder=self.config.is_encoder_decoder
            )
    ...
    def dola_greedy_decode_output_attentions(
            self,
            input_ids: torch.LongTensor,
            tokenizer,
            mature_layer: int,
    ...
    def greedy_search_merge(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # merge
            if not additional_ids is None and generation_step < len(additional_ids):
                next_tokens_scores[:,:] = next_tokens_scores[:,:] + additional_ids[generation_step][:,:]
            # argmax
            next_tokens = torch.argmax(next_tokens_scores, dim=-1)
            generation_step += 1
            save_additional_step.append(next_tokens_scores)
        ...
        if return_dict_in_generate:
            ...
            return input_ids,save_additional_step

    def greedy_search_rag_hidden(
        self,
        input_ids: torch.LongTensor,
        rag_hidden = None,
        logits_processor: Optional[LogitsProcessorList] = None,
    ...
    def greedy_search_constrain_decode(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        this_peer_finished = False  # used by synced_gpus only
        constrain_decode_list = constrain_decode[0].tolist()
        constrain_pre_to_last = {}
        for i in range(len(constrain_decode_list)-1):
            if constrain_decode_list[i] in constrain_pre_to_last.keys():
                constrain_pre_to_last[constrain_decode_list[i]].append(constrain_decode_list[i+1])
            else:
                constrain_pre_to_last[constrain_decode_list[i]] = [constrain_decode_list[i + 1]]
        if not constrain_decode_list[-1] in constrain_pre_to_last.keys():
            constrain_pre_to_last[constrain_decode_list[-1]] = [constrain_decode_list[0]]
        last_id = None
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            # argmax
            # print('token:')
            # print(constrain_decode)
            # print('next_tokens_scores:')
            # print(next_tokens_scores)
            # print(next_tokens_scores.shape)

            # 对张量 A 中不在列表 B 中的索引进行赋值
            # for i in range(next_tokens_scores.shape[1]):
            #     if i not in constrain_decode_list:
            #         next_tokens_scores[0, i] = -10000000
            if last_id is None:
                indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_tokens_scores.shape[1])], dtype=torch.bool)
                next_tokens_scores[:, indices_to_change] = -10000000
            if last_id is not None:
                try:
                    next_tokens_scores[:, constrain_pre_to_last[last_id]] = 10000000*next_tokens_scores[:, constrain_pre_to_last[last_id]]
                except:
                    pass
            #print(next_tokens_scores)
            next_tokens = torch.argmax(next_tokens_scores, dim=-1)
            last_id = next_tokens[:, None][0].cpu().item()
    ...
    def greedy_search_constrain_decode_att(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        synced_gpus: bool = False,
        streamer: Optional["BaseStreamer"] = None,
        dic_rag_att: Optional[torch.Tensor] = None,
        **model_kwargs,
    ) -> Union[GreedySearchOutput, torch.LongTensor]:
        # init values
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        ...
        while True:
            ...
            # indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_tokens_scores.shape[1])], dtype=torch.bool)
            # next_tokens_scores[:, indices_to_change] = -10000000
            # for k,v in dic_rag_att.items():
            #     next_tokens_scores[:,k] += v

            if last_id is None:
                indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_tokens_scores.shape[1])], dtype=torch.bool)
                next_tokens_scores[:, indices_to_change] = -10000000
                for k, v in dic_rag_att.items():
                    next_tokens_scores[:, k] = next_tokens_scores[:, k]*v
            if last_id is not None:
                try:
                    next_tokens_scores[:, constrain_pre_to_last[last_id]] = 10000000*next_tokens_scores[:, constrain_pre_to_last[last_id]]
                    for k, v in dic_rag_att.items():
    ...
    def greedy_search_dynamic_constrain_decode(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            #print('model inputs attention mask: {}'.format(model_inputs['attention_mask'].shape))
            # forward pass to get next token
            outputs = self(
                **model_inputs,
                return_dict=True,
            ...
            class_logits = outputs.logits[1][:,-1,:]
            class_logits = nn.functional.softmax(class_logits,dim=-1)
            #print(class_logits)
            switch_gate = torch.argmax(class_logits, dim=-1)[0]
            go_constrain = False
            #print(class_logits)
            if switch_gate == 0:
            #print('self')
                ...
                #print('constrain')
                go_constrain = True
                indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_token_logits.shape[1])], dtype=torch.bool)
                selected_part = next_token_logits[:, indices_to_change]
                a = 1 + class_logits[:, 2]
                b = 1 - class_logits[:, 2]
                a = a.unsqueeze(-1)
                b = b.unsqueeze(-1)
                # 扩展 a 和 b 到与 selected_part 在第二个维度上相同的大小 #[seq,logits]
                a = a.expand(-1, selected_part.size(1))
                b = b.expand(-1, selected_part.size(1))
                # 选择你要操作的部分
                # 找到小于0的位置并乘以a
                selected_part[selected_part < 0] *= a[selected_part < 0]
                # 找到大于0的位置并乘以b
                selected_part[selected_part > 0] *= b[selected_part > 0]
                # 将修改后的部分赋值回原张量
                next_token_logits[:, indices_to_change] = selected_part

            #next_token_logits = no_rag_token_logits * class_logits[:,0].unsqueeze(-1) + rag_token_logits * class_logits[:,1].unsqueeze(-1) + next_token_logits * class_logits[:,2].unsqueeze(-1)
            ...
            #     indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_tokens_scores.shape[1])], dtype=torch.bool)
            #
            #     selected_part = next_tokens_scores[:, indices_to_change]
            #     a = 1 + class_logits[:, 2]
            #     b = 1 - class_logits[:, 2]
            #     a = a.unsqueeze(-1)
            #     b = b.unsqueeze(-1)
            ...
            #     selected_part[selected_part > 0] *= b[selected_part > 0]
            #     # 将修改后的部分赋值回原张量
            #     next_tokens_scores[:, indices_to_change] = selected_part
    ...
    def greedy_search_constrain_decode_first_token(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
    ...
    def greedy_search_constrain_decode_soft(
        self,
        input_ids: torch.LongTensor,
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            constrain_decode_list = constrain_decode[0].tolist()
            #indices_to_change = torch.tensor([i not in constrain_decode_list for i in range(next_tokens_scores.shape[1])], dtype=torch.bool)
            #next_tokens_scores[:, indices_to_change] = -1000000
            for idx in range(next_tokens_scores.shape[1]):
                if idx in constrain_decode_list:
                    next_tokens_scores[:, idx] = soft_num * next_tokens_scores[:, idx]
            #print(next_tokens_scores)
            next_tokens = torch.argmax(next_tokens_scores, dim=-1)
